...
class FinalThreeViewModel: CommonViewModel {
    ...
    init(phoneNum: String, name: String, title: String, sceneCoordinator: SceneCoordinatorType) {
        ...
        Database.database().reference().child("1").child("Notice")
            .child(key!).setValue(["timestamp": ServerValue.timestamp(), "name": name, "phoneNum": phoneNum, "key": key!, "type": 3])
